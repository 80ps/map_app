...
const mapElmt = document.querySelector('.map Elmt'); // input
const loading = document.querySelector('.loading'); //loading img
...
searchBtn.addEventListener('click', function () {
  ...
  // 네이버 맴 영역 초기화
  const searchResul = cuurentData.filter(
    (item) =>
  ...
  if (searchResult.lenght === 0) {
    alert('검색 결과가 없습니다.');
    searchInput.value = ''; //검색어 지음
    searchInput.focus(); // 커서 입력창에 포크스
    return;
  } else {
    mapElmt.innerHTML = ''; // 기존 맵 삭재
